...
- **Crossplane is now in the dependency surface.** Composition rendering errors, patch DSL mismatches, and Composition/Provider package version drift become new failure modes. Operational cost is bounded by sticking to v1 Composition resources (not Functions / KCL pipeline).
...
2. **Crossplane install.** A platform PR adds `terraform/modules/regional-stack/crossplane.tf`: Crossplane core helm release, `provider-aws` package, and a `ProviderConfig` backed by a platform-side IRSA role (`aegis-platform-aws-crossplane-${region}`). The IRSA gets its own SCP carve-out in landing-zone (sibling of `aegis-platform-aws-ack-iam-*`), its own `/aegis-workload/*` path scoping, and its own trust-subject policy — the four-pack replicated for Crossplane.
