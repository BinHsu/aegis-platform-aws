...
| [ADR-19](19-aws-public-edge-domain-acm-cognito.md) | AWS public edge — domain, ACM, Cognito (WS3) | *Accepted.* Real domain `*.aws.binhsu.org` + ACM TLS + Cognito (per-env user pool, public PKCE client, Hosted UI, pre-token Lambda). Auth terminates at the gateway (OIDC JWT vs Cognito JWKS), not ALB-native. Live-proven on staging 2026-06-18. |
| [ADR-20](20-cognito-tenant-claim-delivery.md) | Cognito tenant claim delivery | *Accepted (2026-06-19).* Pre-Token-Generation Lambda (V1) injects `custom:tenant_id` into the Cognito `id_token`. Groups (option 2) rejected — semantic mismatch; config-only (option 3) does not exist in Cognito; access-token validation (option 4) would require upgrading to Essentials tier. Lambda is the only clean path. Live-proven on staging 2026-06-18. |
| [ADR-21](21-ws3-prod-go-live-followups.md) | WS3 prod go-live forward-fixes | *Proposed (2026-06-18).* Four decoupling fixes surfaced by the prod cold start. **§A** replace Crossplane-composed IRSA with EKS Pod Identity (Terraform-owned); **§B** split `bootstrap_complete` into `bootstrapped` vs `deploy_enabled`; **§C** cut the missing prod image release + per-region ECR seed; **§D** add a TF `test` plan-against-empty-state gate. No code ships in this ADR; each section is independently adoptable (WS4). |
| [ADR-22](22-terraform-crossplane-boundary-v2.md) | Terraform ↔ Crossplane boundary (Crossplane v2) | *Accepted (2026-06-19).* Crossplane v2 (CNCF Graduated GA 2025-08) is the composition engine for workload-scoped cloud resources: WorkloadIdentity XR today, extended on demand. Terraform retains stateful / cluster-scoped resources; Crossplane owns workload-scoped cloud resources with provider-neutral XR vocabulary. The on-prem `provider-minio` fit is a WS4 open validation item. |
...
| [ADR-27](27-node-autoscaling.md) | Node autoscaling — pending pods never scale out | *Accepted (Bin 2026-07-07): Karpenter.* No autoscaler was installed; `desired_size` pinned to `node_min` made `node_max` dead config (#182). Decision: Karpenter (pending-pod provisioning, bin-packing/consolidation, capacity-type fallback that subsumes the #183 On-Demand baseline). Alternatives considered: Cluster Autoscaler, EKS Auto Mode (contradicts the tradeoffs.md deferral), accept-fixed-capacity. Decision only — implementation is a follow-up; #182 stays open to track it. (ADR-24 reserved by in-flight work on another branch; ADR-25/26 reserved by epic #167.) |
