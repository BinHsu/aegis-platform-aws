...
| [ADR-13](13-ci-iam-roles-survive-teardown.md) | CI IAM roles survive teardown-to-zero | *Proposed (needs operator sign-off).* The four CI roles (apply / destroy / plan / greeter-push) + OIDC trust move from `envs/platform` to `envs/bootstrap` (LOCAL state, operator-seeded), structurally removing the destroy-role self-delete hazard, the orphan `EntityAlreadyExists`, and the post-teardown cold-start chicken-egg observed 2026-06-12. Redefines "teardown-to-zero" as zero billable + zero workload (free CI seed roles persist). |
