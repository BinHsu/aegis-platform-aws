...
| [ADR-25](25-node-autoscaling.md) | Node autoscaling — pending pods never scale out | *Proposed — awaiting operator decision (#182).* No autoscaler installed; `desired_size` pinned to `node_min` makes `node_max` dead config. Options: (a) Karpenter (recommended — subsumes the #183 On-Demand baseline via capacity-type fallback), (b) Cluster Autoscaler (minimal fix), (c) EKS Auto Mode (contradicts the tradeoffs.md deferral), (d) remove the dead config and accept fixed capacity. Nothing ships until the operator picks. (ADR-24 is reserved by in-flight work on another branch.) |
