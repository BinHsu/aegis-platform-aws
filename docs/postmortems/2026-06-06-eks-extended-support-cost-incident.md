...
| A2 | **DONE 2026-06-06 (static).** AWS Cost Anomaly Detection (`ce-anomaly.tf`): a DIMENSIONAL/SERVICE monitor + IMMEDIATE subscription that emails when any single anomaly's impact >= $5 — catches an unexpected *shape* of spend (e.g. a new `:extendedSupport` line) before the monthly total trips. us-east-1 aliased provider (CE is global). `terraform validate` clean. **Caveat (verify 6/12):** CE in a member account needs management-enabled member CE access, else the apply fails (the budget is unaffected). | Detection precision | ✅ Static |
...
| A6 | **DONE 2026-06-06 (static).** (1) `apply-regional` self-reaps on a failed apply — `terraform apply \|\| terraform destroy` then re-raise, so a red apply cannot leave billable resources (override: repo var `ALLOW_PARTIAL_APPLY=true` keeps the partial stack for debugging). The kyverno hang this would have hit is fixed by A4. (2) `infra-ops` destroy job has an `if: failure()` step that opens an alert issue — a failed teardown (= still billing) escalates instead of sitting unnoticed (the incident's failed destroy was never re-actioned). `actionlint` clean. | G2, G7 | ✅ Static |
| A7 | **DONE 2026-06-06 (static) — human-gated, not branch-gated (operator decision).** New `gh-tf-destroy-platform` role whose OIDC trust is the GitHub **Environment** subject `environment:destroy` (NOT a branch). `infra-ops.yml` destroy ops moved to a `destroy` job with `environment: destroy` (required reviewer = operator). So ANY branch can dispatch a teardown (fixes the main-only friction that caused attempt #2), but prod is never deleted without an explicit human approval — the boundary is a human, not a branch. Branch-wide trust (`refs/heads/*`) was **rejected**: a destroy role deletes prod by design, so any branch could nuke prod. Permissions = AdministratorAccess for now (the gate is primary control); a **destroy-scoped** policy (deny-create) is an optional follow-up — must be authored empirically from a live destroy's AccessDenied set (destroy does Modify/Detach/Schedule/refresh, not just Delete), not from theory. `terraform validate` + `actionlint` clean; env + `AWS_DESTROY_ROLE_ARN` secret set; role created on next platform apply (6/12). | G6 | ✅ Static |
...
| A9 | **DONE 2026-06-06 (static) — MANUAL mode (operator decision).** Budget Action (`budget.tf`): on a 100%-actual breach AWS Budgets notifies + the action waits in "Requires approval" — it does **not** act until a human approves on the Alert details page (you are emailed first, then you pull the trigger). On approval it attaches a deny policy to the `gh-tf-apply-platform` deploy role (blocks new EKS/EC2/RDS/cache creation; reversible). Scope = deploy role only (stops new spend, doesn't touch running services). **Automatic mode rejected** — auto-restriction in a billing account has too large a blast radius. `terraform validate` clean; live-verifiable on a real breach. | G1 depth | ✅ Static (MANUAL) |
...
| A13 | **DECISION (surfaced 2026-06-06): prod *infrastructure* auto-applies on every merge to main.** `infra-apply.yml` triggers on push to main + `terraform/**`, and with `BOOTSTRAP_COMPLETE=true` runs a real `terraform apply` against **prod** — so a merge re-creates the EKS cluster (billing), with no staging-first or on-demand gate. (The *workload* — greeter — is correctly staging-first via ADR-10 digest promotion; only the *infra* layer auto-applies.) This auto-on-merge is itself a structural contributor: a merge/apply is what stood the incident cluster up. **DONE 2026-06-06 (static):** `infra-apply.yml` changed from `push: main` to **`workflow_dispatch` only** (with a `confirm: "apply"` input + guard). A merge to main no longer auto-deploys prod — the operator deliberately dispatches an apply in a deploy window. PR-time validation still runs via infra-plan; the version-gate + prod-apply-gated environment still gate the dispatched apply. This removes the "merge = prod deploy" footgun and the auto-on-merge that stood the incident cluster up. `actionlint` clean. | G2 / G7, root-cause-adjacent | ✅ Static |
