...
Creates the S3 bucket that the `platform/` and `regional/` envs use as their remote backend, **and** seeds the CI IAM roles (the GitHub OIDC trust + apply / destroy / plan roles, plus the aegis-core frontend push role) — see [ADR-13](../../../docs/adr/13-ci-iam-roles-survive-teardown.md). (A fourth role, the greeter ECR push role, was seeded here too until it was removed 2026-07 — greeter moved to public GHCR, ADR-24.) **No DynamoDB lock table** — TF ≥ 1.11 supports native S3 conditional-write locking via `use_lockfile = true` in each downstream backend block (DynamoDB-based locking is deprecated upstream and will be removed in a future minor version).
...
Only the **state bucket** carries `lifecycle { prevent_destroy = true }` — losing it loses every downstream env's state (irreversible). The CI roles (`iam-seed.tf`, ADR-13) carry **no** such guard: they are cheaply, idempotently recreatable, so a full teardown may delete them and a later seed apply restores them from zero.
...
| **Admin-capable principal for the one-time bootstrap apply** | `AWSControlTowerExecution` (org SCP permits this role to write IAM; SSO principals are denied by the SCP) | Use an IAM admin or break-glass principal. `iam-seed.tf` creates roles, attaches managed policies, and writes inline role policies (e.g. `aws_iam_role_policy.core_frontend`), so the principal needs at minimum `iam:CreateRole`, `iam:AttachRolePolicy`, and `iam:PutRolePolicy`. In practice, using an admin/break-glass role is simplest — a narrowly-scoped principal must cover the full IAM write set. If you have no org SCP blocking IAM writes, your own IAM admin user or SSO admin role works directly. |
...
| **`github_owner` set in `regions.auto.tfvars.json`** | `"github_owner": "BinHsu"` in the committed example. | Set `github_owner` to your GitHub org or username before running `make bootstrap`. This value only wildcards the repo-name segment of the OIDC `sub` (rename-proof); it is no longer the sole binding — see the next row. |
| **`github_platform_repo_id` / `github_core_repo_id` set in `regions.auto.tfvars.json`** (issue #144) | Committed example values are this repo's real ids. | These numeric, immutable GitHub `repository_id`s are the actual trust binding in every OIDC condition in `iam-seed.tf` (`token.actions.githubusercontent.com:repository_id`, StringEquals) — the `sub` claim's repo-name segment is wildcarded so a repo rename/transfer cannot break CI auth (GitHub's 2026-07-15 immutable-subject-claims change). Fetch each with `gh api repos/<owner>/<repo> --jq .id` — **never guess it**; a wrong id fails closed (every CI OIDC assume-role starts failing), it does not silently over-trust. (`github_greeter_repo_id` was removed 2026-07 along with the greeter ECR push role — greeter moved to public GHCR, ADR-24.) |
...
1. **Operator (break-glass / admin principal), local laptop** — run `make bootstrap ENV=<staging|prod>`. This selects the per-account workspace (issue #90), then creates the state bucket and all CI roles in one apply (`iam-seed.tf`). Precondition: the GitHub OIDC provider row above is satisfied. Run once per account, each with its own `ENV`.
...
| `github-actions-aegis-core-frontend` | aegis-core S3 frontend sync + CloudFront invalidation, main-ref only. `iam-seed.tf`. |
| ~~`aegis-greeter-ci`~~ | *(removed 2026-07)* aegis-greeter ECR push, main-ref only. Greeter moved to public GHCR (ADR-24); this role never successfully assumed anyway (trust policy never wired correctly). |
